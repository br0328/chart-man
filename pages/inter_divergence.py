...
out_tab = get_out_tab({
	'Plot': html.Div([html.Div(id = 'out-plot-1'), html.Div(id = 'out-plot-2')]),
	'Report': get_report_div()
})
layout = get_page_layout('Inter Divergence', scenario_div, None, out_tab)
...
@callback(
	[
		...
		Output('out-plot-1', 'children', allow_duplicate = True),
        Output('out-plot-2', 'children', allow_duplicate = True),
		Output('out-report', 'children', allow_duplicate = True)
	],
	Input('inter-diver-analyze-button', 'n_clicks'),
	[
...
def on_analyze_clicked(n_clicks, symbol1, symbol2, from_date, to_date):
    none_ret = [None, None, None] # Padding return values

    if n_clicks == 0: return alert_hide(none_ret)
	
    if symbol1 is None: return alert_error('Invalid first symbol. Please select one and retry.', none_ret)
    if symbol2 is None: return alert_error('Invalid second symbol. Please select one and retry.', none_ret)
